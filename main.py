...
def register():
    form = RegisterForm()
# creating new form for new user and generating hash password with WTF form, we have to use form.data
...
# if the password is not correct or the email that has been registered into the database
        if not user:
            flash('please email does not exist, please try again')
            return redirect(url_for('login'))
...
if __name__ == "__main__":
    app.run(host='localhost', port=1983, debug=True)
